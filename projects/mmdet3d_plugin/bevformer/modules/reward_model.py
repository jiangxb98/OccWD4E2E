...
class RewardConvNet(nn.Module):
    def __init__(self, 
                 ...
                 sim_head_type: str = 'ALL',  # NC, DAC, TTC, EP, Comfortability, ALL, WO_COMF
    ...
    def reward_distillation_alignment_with_sim(self, model_a_trajectories, model_b_trajectory, fut_bev_feature, return_distance_loss=False, sim_reward_weight=1.0):
        ...
        # 编码模型a的轨迹
        model_a_trajectories_embed = model_a_trajectories.reshape(-1, 2)  # [bs*times*N, 2]
        model_a_trajectories_embed = self.trajectory_single_encoder(model_a_trajectories_embed)  # [bs*times*N, 256]
        model_a_trajectories_embed = model_a_trajectories_embed.reshape(bs, times, num_traj, 256)
        ...
        # 使用simulation reward model评估模型a的轨迹
        sim_traj_scores_a_list = []
        ...
        # 为每个sim reward选择最佳轨迹
        best_traj_idx_sim = torch.argmax(sim_traj_scores_a, dim=3)  # [bs, times, sim_reward_nums]
        best_traj_a = model_a_trajectories[:, torch.arange(times), best_traj_idx_sim[:, :, 0].squeeze(0)]  # [bs, times, 2] 使用第一个sim reward的最佳轨迹
